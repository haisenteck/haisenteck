...
static void subghz_scene_receiver_update_statusbar(void* context) {
    ...
    if(!subghz_history_get_text_space_left(subghz->history, history_stat_str, subghz->gps->satellites)) {
...
static void subghz_scene_add_to_history_callback(SubGhzReceiver* receiver, SubGhzProtocolDecoderBase* decoder_base, void* context) {
	...
	//-----------------------------------------------------------------------------------------------------------------------
		...
		if(subghz->last_settings->autosave_text == true) {
			notification_message(subghz->notifications, &subghz_sequence_rx);
			subghz->last_settings->autosave_auto = true;
...
void subghz_scene_receiver_on_enter(void* context) {
    ...
    subghz_view_receiver_set_callback(subghz->subghz_receiver, subghz_scene_receiver_callback, subghz);
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        switch(event.event) {
			...
			case SubGhzCustomEventViewReceiverConfig:
				...
				scene_manager_set_scene_state(subghz->scene_manager, SubGhzViewIdReceiver, SubGhzCustomEventManagerSet);
        ...
        SubGhzThresholdRssiData ret_rssi = subghz_threshold_get_rssi_data(subghz->threshold_rssi, subghz_txrx_radio_device_get_rssi(subghz->txrx));
